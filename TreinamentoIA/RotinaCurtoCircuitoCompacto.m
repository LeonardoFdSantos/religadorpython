...
for dd =[5:1:120]
    ...
    valores_resultados_fim_linha_sem_compensacao(1, :) = string({'dd' 'tipoCurto' 'IA_Sim' 'IB_Sim' 'IC_Sim' 'IA_Calc' 'IB_Calc' 'IC_Calc' 'VA_Sim' 'VB_Sim' 'VC_Sim' 'VA_Calc' 'VB_Calc' 'VC_Calc' 'VA_Entrada_TRIF' 'VB_Entrada_TRIF' 'VC_Entrada_TRIF'});
    valores_resultados_meio_linha_sem_compensacao(1, :) = string({'dd' 'n' 'tipoCurto' 'IA_Sim' 'IB_Sim' 'IC_Sim' 'IA_Calc' 'IB_Calc' 'IC_Calc' 'VA_Sim' 'VB_Sim' 'Vc_Sim' 'VA_Calc' 'VB_Calc' 'VC_Calc' 'VA_Entrada_TRIF' 'VB_Entrada_TRIF' 'VC_Entrada_TRIF'});
    
    valores_resultados_simulacao_meio_linha_sem_comp(1, :) = string({'dd' 'n' 'tipoCurto' 'Raf' 'Rbf' 'Rcf' 'RaTrif' 'RbTrif' 'RcTrif' 'IA_T2F' 'IB_T2F' 'IC_T2F' 'IA_TRIF' 'IB_TRIF' 'IC_TRIF' 'VA_T2F' 'VB_T2F' 'VC_T2F' 'VA_TRIF' 'VB_TRIF' 'VC_TRIF'});
    valores_resultados_simulacao_fim_linha_sem_comp(1, :) = string({'dd' 'tipoCurto' 'Raf' 'Rbf' 'Rcf' 'RaTrif' 'RbTrif' 'RcTrif' 'IA_T2F' 'IB_T2F' 'IC_T2F' 'IA_TRIF' 'IB_TRIF' 'IC_TRIF' 'VA_T2F' 'VB_T2F' 'VC_T2F' 'VA_TRIF' 'VB_TRIF' 'VC_TRIF'});
    
    % Parametros_testes = [0.001 .1 .15 .2 .25 .3 .35 .4 .45 .5 .55 .6 .65 .7 .75 .8 .85 .9 .95 .999];
    % Parametros_testes = [0.001 .1 .2 .3 .4 .5 .6 .7 .8 .9 .999];
    % Parametros_testes = [0.01 0.25 0.5 0.75 0.99];
    ...
    for b = [1:1:5]
        ...
        TensaoCalculado = abs([Va Vb Vc]/sqrt(2));
        valores_resultados_fim_linha_sem_compensacao(b+1, :) = [dd tipoCurto Corrente_T2F_Ensaio IA IB IC TensaoCalculado Tensao_T2F_Ensaio Tensao_Trif_Ensaio];
    
    end
    
    fprintf('Terminado fim de Linha! Distância de %2.f \n', dd);
    b = 0;
    c = 1;
    e = 1;
    ...
    for n = Parametros_testes
        ...
        for b = [1:1:5]
            ...
            TensaoCalculado = abs([Va Vb Vc])/sqrt(2);
            valores_resultados_meio_linha_sem_compensacao(c+1, :) = [dd m1 tipoCurto Corrente_T2F_Ensaio IA IB IC TensaoCalculado Tensao_T2F_Ensaio Tensao_Trif_Ensaio];
            c = c + 1;
        end
    end
    fprintf('Terminado meio de Linha! Distância de %2.f \n', dd);
end
